#!/usr/bin/env node
/**
 * render-cli-v6.js — Fingerprint dossier renderer
 * Dual font weights (bold = inferences, light = data) + data-conditional + img2sound
 * Lines prefixed with ! render in bold, all others render in light/regular
 *
 * Usage:  node render-cli-v6.js <input.json> [output.mp4]
 */
...
// ---- V3-style smooth timing (deterministic, no randomness) ----
// Based on line count and importance — gives the deliberate, breathing feel of v3
function smoothTiming(lineCount, importance) {
  // Base hold: 0.7s for 1 line, +0.25s per extra line
  const base = 0.7 + Math.max(0, lineCount - 1) * 0.25;
  // Importance multiplier: flash is quick, linger is slow, normal is standard
  if (importance === 'flash') return Math.max(0.5, base * 0.6);
...
// ---- Fit font size (weight-aware) ----
function fitFontSize(ctx, text, maxW, maxSize, minSize, weight) {
  const w = weight || 'bold';
  for (let s = maxSize; s >= minSize; s -= 2) {
    ctx.font = `${w} ${s}px "Neue Montreal"`;
...
async function main() {
  ...
  if (args.length < 1) {
    console.error('Usage: node render-cli-v6.js <input.json> [output.mp4]');
    process.exit(1);
  }

  const inputPath = path.resolve(args[0]);
  const outputPath = args[1] ? path.resolve(args[1]) : inputPath.replace(/\.json$/i, '-v6.mp4');
  ...
  for (let i = 0; i < blocks.length; i++) {
    blocks[i].holdSec = smoothTiming(blocks[i].lines.length, blocks[i].importance);
  }
  ...
  for (let gf = 0; gf < totalFrames; gf++) {
    ...
    for (let bi = 0; bi < blocks.length; bi++) {
      ...
      for (const line of block.lines) {
        if (line === '') lineSizes.push({ size: 40, text: '', weight: 'normal' });
        else {
          const isBold = line.startsWith('!');
          const cleanText = isBold ? line.slice(1) : line;
          const weight = isBold ? 'bold' : 'normal';
          const sz = fitFontSize(ctx, cleanText, maxTextW, 120, 28, weight);
          lineSizes.push({ size: sz, text: cleanText, weight });
        }
      }
      ...
      for (let li = 0; li < block.lines.length; li++) {
        ...
        ctx.font = `${ls.weight} ${ls.size}px "Neue Montreal"`;
      ...
      if (f < contentEnd && blockAlpha > 0.5) {
        ...
        if (blink && visibleLines > 0) {
          ...
          ctx.font = `${lastLS.weight} ${lastLS.size}px "Neue Montreal"`;
  ...
  if (fs.existsSync(img2sound)) {
    console.log('\n  Sonifying with img2sound (additive)...');
    try {
      const out = execSync(
        `"${img2sound}" additive "${silentPath}" --freq-min 40 --freq-max 1400 --oscillators 96`,
      ...
      for (const dir of dirs) {
        ...
        if (wavs.length > 0) { wavPath = path.join(dirPath, wavs[0]); break; }
      }
    } catch(e) {
      console.log('  img2sound error: ' + e.message.slice(0, 100));
    }
  }

  // ---- Merge video + sonified audio ----
  if (wavPath && fs.existsSync(wavPath)) {
    ...
    console.log('  No audio generated, using silent video.');
    fs.renameSync(silentPath, outputPath);
  }
