 ...
 * V3: More internal monolog — deeper robot personality, uncomfortable inferences, punchy thoughts
 *
...
function buildMonolog(fpData, profile, entropy, uniqueness, pricing) {
  ...
  // 2b - expensive taste
  blocks.push({
    lines: ['someone invested in this.'],
    hold: 1.0,
  });
  ...
  // 5b - false sense of security
  blocks.push({
    lines: [
      'thinks incognito means invisible.',
    ],
    hold: 1.2,
  });
  ...
  if (battery.level !== undefined) {
    ...
    if (charging) {
      blocks.push({
        lines: ['settled in.', 'not going anywhere.'],
        hold: 1.0,
      });
    }
  }
  ...
  if (localHour !== null) {
    ...
    if (h >= 22 || h < 6) {
      ...
      blocks.push({
        lines: [
          'tired. guard is down.',
          'emotional purchases peak at 8pm.',
        ],
        hold: 1.2,
      });
    }
  ...
  if (langInForeign) {
    ...
    // 10b - expat exploitation
    blocks.push({
      lines: [
        'probably misses home.',
        'show airline ads.',
      ],
      hold: 1.2,
    });
  ...
  if (devFonts.length > 0 || designFonts.length > 0) {
    ...
    if (devFonts.length > 0 && designFonts.length > 0) {
      blocks.push({
        lines: [
          'builds things and makes them pretty.',
          'rare combination.',
          'charge more.',
        ],
        hold: 1.3,
  ...
  if (cookies) {
    ...
    blocks.push({
      lines: [
        'you agreed to this.',
        'somewhere in a terms of service.',
        'you didn\'t read.',
      ],
      hold: 1.5,
    });
    ...
    if (zombies > 0) {
      ...
      blocks.push({
        lines: [
          'deleted your cookies last week?',
          '',
          'they came back.',
        ],
        hold: 1.5,
...
async function main() {
  ...
  if (args.length < 1) {
    console.error('Usage: node render-cli-v3.js <input.json> [output.mp4]');
    process.exit(1);
  }

  const inputPath = path.resolve(args[0]);
  const outputPath = args[1] ? path.resolve(args[1]) : inputPath.replace(/\.json$/i, '-v3.mp4');
