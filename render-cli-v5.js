#!/usr/bin/env node
/**
 * render-cli-v5.js — Fingerprint dossier renderer
 * V4 data-conditional blocks + V3-style smooth timing + img2sound sonification
 *
 * Usage:  node render-cli-v5.js <input.json> [output.mp4]
 */
...
async function main() {
  ...
  if (args.length < 1) {
    console.error('Usage: node render-cli-v5.js <input.json> [output.mp4]');
    process.exit(1);
  }

  const inputPath = path.resolve(args[0]);
  const outputPath = args[1] ? path.resolve(args[1]) : inputPath.replace(/\.json$/i, '-v5.mp4');
